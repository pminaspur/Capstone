...
class AnimalShelter(object):
    ...
    def __init__(self, mongo_database, mongo_collection):
...
# this create method to implement the C in CRUD and insert the date using insert one method
    ...
    def generate_dog_report(self, data):
        if data is not None:
            dog_report = []
            for animal in self.database.animals.aggregate(data):
                dog_report.append(animal)
            return dog_report
            
        else:
            raise Exception("No report generated because data parameter is empty")
